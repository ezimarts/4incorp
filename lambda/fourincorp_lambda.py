...
def initialize_intake(application):
    ...
    application["form_record_key"] = prefix + "intake/application.json"
    ...
    s3.put_object(Bucket=INTAKE_BUCKET, Key=application["form_record_key"],
                  Body=json.dumps(application, default=json_default).encode("utf-8"),
...
def create_document_upload(event, reference):
    ...
    try:
        key = intake_prefix(application) + f"documents/{document_id}/{file_name}"
    except ValueError as error:
        return response(400, {"message": str(error)})
